...
class ToolReplacementProcessor:
    def __init__(self):
        # Fixed: Use parent (not parent.parent) since script is in project root
        self.project_root = Path(__file__).parent

        # Try to find toolData.js in multiple possible locations
        possible_paths = [
            self.project_root / 'src' / 'app' / 'utils' / 'toolData.js',
            self.project_root / 'frontend' / 'src' / 'app' / 'utils' / 'toolData.js'
        ]

        self.tool_data_path = None
        for path in possible_paths:
            if path.exists():
                self.tool_data_path = path
                break

        if not self.tool_data_path:
            print(f"❌ Could not find toolData.js in any of these locations:")
            for path in possible_paths:
                print(f"   - {path}")
            raise FileNotFoundError("toolData.js not found in expected locations")
    ...
    def apply_replacements(self):
        ...
        if not self.tool_data_path.exists():
            print(f"❌ Error: toolData.js not found at {self.tool_data_path}")
